...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const subs = await api(`vita_push_subscriptions?select=*&owner_id=eq.${encodeURIComponent(user.id)}&enabled=eq.true`);
    let sent = 0;
    let failed = 0;
...
async function getUser(req) {
  const res = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
    headers: { apikey: ANON_KEY || SERVICE_ROLE, Authorization: req.headers.get("Authorization") ?? "" }
  });
