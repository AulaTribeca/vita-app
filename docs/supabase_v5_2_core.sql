...
-- VITA v5.2, datos base de módulos oficiales sin duplicar.

-- Activos iniciales del hogar.
insert into public.vita_cards (owner_id, household_id, visibility, module, category, title, details, status, payload)
select min(hm.user_id::text)::uuid, hm.household_id, 'household', 'home_assets', 'home', 'Casa', 'Activo del hogar', 'open', jsonb_build_object('type','home')
from public.household_members hm
where hm.status = 'active'
group by hm.household_id
having not exists (
  select 1
  from public.vita_cards v
  where v.household_id = hm.household_id
  and v.module = 'home_assets'
  and lower(v.title) = lower('Casa')
);

insert into public.vita_cards (owner_id, household_id, visibility, module, category, title, details, status, payload)
select min(hm.user_id::text)::uuid, hm.household_id, 'household', 'home_assets', 'home', 'Piso', 'Activo del hogar', 'open', jsonb_build_object('type','flat')
from public.household_members hm
where hm.status = 'active'
group by hm.household_id
having not exists (
  ...
  where v.household_id = hm.household_id
  and v.module = 'home_assets'
  and lower(v.title) = lower('Piso')
);

-- Tarjeta/módulo compra compartida inicial.
insert into public.vita_cards (owner_id, household_id, visibility, module, category, title, details, status, payload)
select min(hm.user_id::text)::uuid, hm.household_id, 'household', 'shopping', 'shopping', 'Lista de la compra compartida', 'Compra común del hogar', 'open', jsonb_build_object('type','shared_list')
from public.household_members hm
where hm.status = 'active'
group by hm.household_id
having not exists (
  ...
  from public.vita_cards v
  where v.household_id = hm.household_id
  and v.module = 'shopping'
  and lower(v.title) = lower('Lista de la compra compartida')
);
