-- VITA v4.2
-- Paneles clicables, salud restaurada y activos del hogar.
-- Ejecutar completo en Supabase SQL Editor.
-- Sustituye a todos los SQL v4 anteriores.
...
alter table public.health_records
  ...
  add column if not exists record_type text default 'note',
...
alter table public.household_assets
  add column if not exists household_id uuid references public.households(id) on delete cascade,
...
-- =========================
-- LISTAS, compatibles con estructura antigua y nueva
-- =========================
...
create table if not exists public.shopping_lists (
  ...
  name text default 'Lista',
  list_type text default 'personal',
  title text default 'Lista',
  active boolean default true,
  created_at timestamptz default now(),
  updated_at timestamptz default now()
);
...
alter table public.shopping_lists
  ...
  add column if not exists name text default 'Lista',
  add column if not exists list_type text default 'personal',
  add column if not exists title text default 'Lista',
  add column if not exists active boolean default true,
  add column if not exists created_at timestamptz default now(),
  add column if not exists updated_at timestamptz default now();
...
set
  name = coalesce(nullif(name, ''), title, 'Lista'),
  title = coalesce(nullif(title, ''), name, 'Lista'),
  list_type = coalesce(nullif(list_type, ''), 'personal'),
...
alter table public.shopping_lists
  ...
  alter column list_type set default 'personal',
  alter column visibility set default 'private',
  alter column active set default true;

alter table public.shopping_lists drop constraint if exists shopping_lists_list_type_check;
alter table public.shopping_lists drop constraint if exists shopping_lists_visibility_check;
...
begin
  if not exists (
    ...
    where conrelid = 'public.shopping_lists'::regclass
      and conname = 'shopping_lists_list_type_check'
  ) then
    alter table public.shopping_lists
    add constraint shopping_lists_list_type_check
    check (list_type in ('personal','private','household','shared','shopping','wishlist'));
  end if;

  if not exists (
    select 1 from pg_constraint
    where conrelid = 'public.shopping_lists'::regclass
      and conname = 'shopping_lists_visibility_check'
  ) then
    alter table public.shopping_lists
    add constraint shopping_lists_visibility_check
    check (visibility in ('private','shared','household','selected'));
  end if;
end $$;
...
create table if not exists public.shopping_list_items (
  ...
  title text default 'Elemento',
  name text default 'Elemento',
  quantity text,
  notes text,
  url text,
  image_path text,
...
alter table public.shopping_list_items
  ...
  add column if not exists title text default 'Elemento',
  add column if not exists name text default 'Elemento',
  add column if not exists quantity text,
  add column if not exists notes text,
  add column if not exists url text,
  add column if not exists image_path text,
  add column if not exists image_url text,
  add column if not exists price_estimate numeric(10,2),
...
set
  title = coalesce(nullif(title, ''), name, 'Elemento'),
  name = coalesce(nullif(name, ''), title, 'Elemento'),
  created_by = coalesce(created_by, owner_id),
  owner_id = coalesce(owner_id, created_by),
  checked = coalesce(checked, false),
  status = coalesce(nullif(status, ''), case when checked then 'bought' else 'pending' end);
...
begin
  if not exists (
    ...
    where conrelid = 'public.shopping_list_items'::regclass
      and conname = 'shopping_list_items_status_check'
  ) then
    alter table public.shopping_list_items
    add constraint shopping_list_items_status_check
    check (status in ('pending','bought','archived','done'));
  end if;
end $$;
...
drop policy if exists "health_records_all_own" on public.health_records;
...
using (
  ...
  or (visibility in ('shared','household') and public.is_household_member(household_id, auth.uid()))
);
...
using (
  exists (
    ...
    and (
      l.owner_id = auth.uid()
      or (l.visibility in ('shared','household') and public.is_household_member(l.household_id, auth.uid()))
...
-- Una única lista de compra compartida por hogar, compatible con esquema antiguo.
insert into public.shopping_lists (owner_id, household_id, visibility, name, list_type, title, active)
select min(hm.user_id::text)::uuid, hm.household_id, 'shared', 'Lista de la compra', 'household', 'Lista de la compra', true
from public.household_members hm
...
having not exists (
  ...
  and l.list_type in ('household','shared','shopping')
  and l.visibility in ('shared','household')
);

-- Lista privada por usuario.
insert into public.shopping_lists (owner_id, household_id, visibility, name, list_type, title, active)
select hm.user_id, hm.household_id, 'private', 'Mi lista privada', 'personal', 'Mi lista privada', true
from public.household_members hm
where hm.status = 'active'
and not exists (
  select 1 from public.shopping_lists l
  where l.owner_id = hm.user_id
  and l.list_type in ('personal','private')
  and l.visibility = 'private'
);

-- Lista de deseos por usuario.
insert into public.shopping_lists (owner_id, household_id, visibility, name, list_type, title, active)
select hm.user_id, hm.household_id, 'private', 'Mis deseos', 'wishlist', 'Mis deseos', true
from public.household_members hm
where hm.status = 'active'
and not exists (
  select 1 from public.shopping_lists l
...
set created_by = coalesce(i.created_by, i.owner_id, l.owner_id),
    owner_id = coalesce(i.owner_id, i.created_by, l.owner_id)
from public.shopping_lists l
where i.list_id = l.id
and (i.created_by is null or i.owner_id is null);

-- Activos iniciales, sin duplicar.
insert into public.household_assets (household_id, created_by, name, asset_type, status, active)
select hm.household_id, min(hm.user_id::text)::uuid, 'Casa', 'home', 'active', true
from public.household_members hm
where hm.status = 'active'
group by hm.household_id
having not exists (
  select 1 from public.household_assets a
  where a.household_id = hm.household_id
  and lower(a.name) = lower('Casa')
);

insert into public.household_assets (household_id, created_by, name, asset_type, status, active)
select hm.household_id, min(hm.user_id::text)::uuid, 'Piso', 'flat', 'active', true
from public.household_members hm
where hm.status = 'active'
group by hm.household_id
having not exists (
...
create index if not exists health_records_owner_date_idx on public.health_records(owner_id, occurred_at);
...
create index if not exists household_assets_household_idx on public.household_assets(household_id, active);
...
create index if not exists shopping_items_owner_idx on public.shopping_list_items(owner_id);
